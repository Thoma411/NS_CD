...
LastEditTime: 2023-04-30 23:20:09
...
def sRecv(sk: socket, MAX_SIZE: int, st: str):
    while True:
        ...
        print('recvlen:', len(sMsgRecv[0].decode()))


def sMain():
    serverSk = socket(AF_INET, SOCK_DGRAM)  # 创建套接字
    # serverPort = sm.U_sPort
    src_Port = int(input('src Port: '))
    serverSk.bind(('', src_Port))  # 绑定IP/端口
    ...
    tsSend = th.Thread(target=sSend, args=(serverSk, dst_IP, dst_Port, s_t))
    tsRecv = th.Thread(target=sRecv, args=(serverSk, MAX_SIZE, s_t))
    tsSend.start()
    tsRecv.start()
