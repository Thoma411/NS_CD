...
class ArrangeSimpleDES():
    def __init__(self):
        ...
        self.k0 = [1, 1, 2, 2, 2, 2, 2, 2, 1,
                   2, 2, 2, 2, 2, 2, 1, ]  # 秘钥循环移位的位数
    ...
    def bin2str(self, binary: str) -> str:
        ...
        :return:
        """
        list_bin = [binary[i:i + 8]
                    for i in range(0, len(binary), 8)]  # 对二进制字符串进行切分, 每8位为一组
        list_int = []
        for b in list_bin:
    ...
    def decrypt(self, ciphertext):
        ...
        key_list = self.__get_key_list()
        key_list = key_list[::-1]
        bin_ciphertext_block = [bin_ciphertext[i:i + 64]
                                for i in range(0, len(bin_ciphertext), 64)]
